...
func addMigration(name string, customDir string) error {
	...
	if err := os.WriteFile(filepath.Clean(upFile), []byte(upContent), 0o644); err != nil {
	...
	if err := os.WriteFile(filepath.Clean(downFile), []byte(downContent), 0o644); err != nil {
...
func convertMySQLURL(url string) string {
	...
	before, after, ok := strings.Cut(hostPath, "/")
	if !ok {
