...
func highlightMatch(s string, start, end int) string {
	// Highlight the matched portion in yellow/bold
	if start < 0 || end > len(s) || start >= end {
		return s
	}

	return s[:start] + "\033[1;33m" + s[start:end] + "\033[0m" + s[end:]
}
...
func runStreamSearch(c *client.Client, req *client.SearchRequest, output string) error {
	...
	var pendingResult *client.SearchResult
	var pendingRanges [][2]int
