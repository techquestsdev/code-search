...
# - nodejs, npm: for scip-typescript and scip-python (via npx)
# - python3: for scip-python to analyze Python code
RUN apk add --no-cache ca-certificates tzdata git jansson \
    nodejs npm \
    python3

# Pre-install SCIP tools globally so they're available without npx download
