...
export function createHoverExtension(
  ...
  onHoverStateChange: (
    state: HoverState | null | ((prev: HoverState | null) => HoverState | null)
  ) => void,
  ...
  return EditorView.domEventHandlers({
    mousemove: (event, view) => {
      ...
      hoverTimeout = setTimeout(async () => {
        ...
        const signature = await fetchInfo(
          wordInfo.word,
          wordInfo.line,
          wordInfo.startCol
        );
        ...
        onHoverStateChange((prev: HoverState | null) => {
          if (
            prev &&
            prev.word === wordInfo.word &&
            prev.line === wordInfo.line
          ) {
...
export function isReservedKeyword(word: string, language?: string): boolean {
  ...
  if (
    (lang === "tsx" || lang === "jsx") &&
    RESERVED_KEYWORDS["typescript"]?.has(word)
  )
    return true;
...
export function createLineNumberClickExtension(
...
export function createHighlightExtension(highlightLines: number[]): Extension {
